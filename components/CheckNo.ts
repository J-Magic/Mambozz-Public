...
export default function formatPhoneNumber(phoneNumber: string) {
  try {
    ...
    // Format the phone number to the international standard (e.g., +1 123-456-7890)
    const formattedPhoneNumber = parsedPhoneNumber?.formatInternational();

    console.log('FORMATTED PHONE NUMBER:', formattedPhoneNumber);

    const phoneNumberCleaned = formattedPhoneNumber?.replace(/[\s-]/g, '');

    console.log('PHONE NUMBER CLEANED:', phoneNumberCleaned); // Output: "+11234567890"
    ...
    return phoneNumber; // Return the original phone number on error
  }
}
